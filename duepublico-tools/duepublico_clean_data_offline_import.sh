#!/bin/bash
logtemplate=" - duepublico_clean_data_offline_import.sh:"
...
if [[ -z "$mcr_data_directory" ]]; then

	printf '%s This script requires flag -d (mcr data directory) -> please make it available\n' "$(date) $logtemplate"
	exit 1
fi
