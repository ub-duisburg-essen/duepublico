#!/bin/bash
logtemplate=" - duepublico_provide_prod_2021.06.sh:"
current_dir=$(pwd)
...
mkdir ./env/tmp
mkdir ./env/tmp/prod
mkdir ./env/tmp/prod/data
mkdir ./env/tmp/prod/db

# create dump for postgres db
printf '%s Create dump for postgres db\n' "$(date) $logtemplate"
pg_dump >./env/tmp/prod/db/duepublico_prod.sql

# copy current mcrhome
printf '%s Copy current duepublico home\n' "$(date) $logtemplate"
cp -rp ~/.mycore/duepublico ./env/tmp/prod

# copy all metadata
printf '%s Copy metadata\n' "$(date) $logtemplate"
cp -rp $data_directory/metadata ./env/tmp/prod/data/

printf '%s Copy versions-metadata\n' "$(date) $logtemplate"
# copy all versions_metadata
cp -rp $data_directory/versions-metadata ./env/tmp/prod/data/

printf '%s Create dummy content\n' "$(date) $logtemplate"
# create dummy data
mkdir ./env/tmp/prod/data/content
...
cd $current_dir/env/tmp/prod/data/content
...
tar cfz duepublico_prod.tar.gz ./prod

# encrypt out.tar.gz
printf '%s Encrypt duepublico_prod.tar.gz\n' "$(date) $logtemplate"
openssl enc -aes-256-cbc -md sha512 -pbkdf2 -iter 1000000 -salt -in ./duepublico_prod.tar.gz -out ./$name_out -pass file:../dc.txt
