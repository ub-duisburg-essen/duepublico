...
if ! [ -f ../../duepublico-webapp/target/duepublico-webapp-2023.06.5-SNAPSHOT.war ]; then

  printf '%s This script needs a maven build of duepublico - please make them available!\n' "$(date) $logtemplate"
  exit 1
fi

# Is there needed certificate ?
if ! [ -f ./env/cert/HARICA-TLS-Root-2021-RSA.pem ]; then
...
printf '%s Import cert with keytool \n' "$(date) $logtemplate"

# Execute the keytool import command
docker exec duepublico-2023-war keytool -import \
  -trustcacerts \
  -noprompt \
  -file /root/cert/HARICA-TLS-Root-2021-RSA.pem \
  -keystore /usr/local/openjdk-17/lib/security/cacerts \
  -storepass $(prop STORE_PASS)

# Check the exit status of the previous command ($?)
if [ $? -eq 0 ]; then
  printf '%s Success: Certificate imported. Restarting container duepublico-2023-war \n' "$(date) $logtemplate"
  docker restart duepublico-2023-war
else
  printf '%s Error: Certificate import failed! \n' "$(date) $logtemplate"
  exit 1
fi
