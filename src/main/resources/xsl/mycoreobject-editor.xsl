  ...
  <xsl:template match="/mycoreobject" priority="10">
    <xsl:copy>
      ...
      <xsl:if test="not(@ID) or acl:checkPermission(@ID,'writedb')">
        ...
        <xsl:for-each select="structure[parents|derobjects]">
          <xsl:copy>
            <xsl:if test="parents/parent">
              <parents class="MCRMetaLinkID">
                <xsl:apply-templates select="parents/parent" mode="editor" />
              </parents>
            </xsl:if>
            <xsl:for-each select="derobjects">
              <xsl:copy>
                <xsl:attribute name="class">MCRMetaDerivateLinkID</xsl:attribute>
                <xsl:copy-of select="*" />
              </xsl:copy>
            </xsl:for-each>
          </xsl:copy>
