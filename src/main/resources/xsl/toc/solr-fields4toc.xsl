...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="mycoreobject">
    ...
    <xsl:for-each select="metadata/def.modsContainer/modsContainer/mods:mods">
      ...
      <xsl:apply-templates select="mods:relatedItem" mode="toc" />
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template match="mods:relatedItem[contains('host series',@type)]" mode="toc">
    <xsl:for-each select="mods:part">
      <xsl:apply-templates select="@order" mode="toc.field">
        <xsl:with-param name="name">order</xsl:with-param>
      </xsl:apply-templates>
      <xsl:apply-templates select="mods:detail[@type]/mods:number" mode="toc.field" />
      <xsl:apply-templates select="mods:extent[@unit='pages']/mods:start" mode="toc.field">
        <xsl:with-param name="name">page</xsl:with-param>
      </xsl:apply-templates>
    </xsl:for-each>
    
    <xsl:apply-templates select="@xlink:href" mode="toc" />
    <xsl:apply-templates select="mods:relatedItem" mode="toc" />
  </xsl:template>
