...
-->
 ...
 <xsl:template match="mods:classification[contains(@authorityURI,'classifications/collection')]"/>
  ...
  <xsl:template match="mods:classification[(@authority='marcgt') or contains(@authorityURI,'/XMetaDissPlusThesisLevel')]">
  ...
  <xsl:template match="mods:mods" mode="dc_date">
    <xsl:choose>
      <xsl:when test="mods:originInfo[@eventType='publication']/mods:dateIssued[@point='start']">
        <xsl:apply-templates select="mods:originInfo[@eventType='publication']/mods:dateIssued[@point='start']" />
      </xsl:when>
