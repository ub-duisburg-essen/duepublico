...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xalan"
    ...
    <xsl:choose>
      <xsl:when test="string-length($classlink) &gt; 0">
        <xsl:for-each select="document($classlink)/mycoreclass">
          <tr>
            <td valign="top" class="metaname">
              <xsl:call-template name="duepublico.output.label" />
              <xsl:text>:</xsl:text>
            </td>
            <td class="metavalue">
      ...
      <xsl:otherwise>
        <tr>
          <td valign="top" class="metaname">
            <xsl:value-of select="i18n:translate('component.mods.metaData.dictionary.classification')" />
            <xsl:text>:</xsl:text>
          </td>
          <td class="metavalue">
            <xsl:apply-templates select="categories/category" mode="printModsClassInfo" />
          </td>
        </tr>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template name="duepublico.output.label">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:value-of select="label[not(contains(@xml:lang,'x-'))][1]/@text" />
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
