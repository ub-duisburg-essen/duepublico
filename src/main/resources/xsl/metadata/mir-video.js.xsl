...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mcr="xalan://org.mycore.common.xml.MCRXMLFunctions"
  ...
  <xsl:param name="MCR.Video.fetchSubtitles"/>
  ...
  <xsl:template match="doc" mode="resultsByDerivate">
    ...
    <xsl:choose>
      <xsl:when test="$fileMimeType = 'video/mp4'">
        <option data-file-extension="{$lowercaseExtension}" data-audio="false" fetch-subtitles="{$MCR.Video.fetchSubtitles}"
      ...
      <xsl:otherwise>
        <option data-file-extension="{$lowercaseExtension}" data-mime-type="{$fileMimeType}"
          data-src="{concat($ServletsBaseURL, 'MCRFileNodeServlet/', $derivateID, '/', $filePath)}" data-audio="true" fetch-subtitles="{$MCR.Video.fetchSubtitles}"
          data-is-main-doc="{mcr:getMainDocName($derivateID)=$filePath}">
          <xsl:value-of select="$fileName" />
        </option>
      </xsl:otherwise>
