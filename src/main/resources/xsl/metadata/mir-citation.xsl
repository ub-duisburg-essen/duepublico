  ...
  <xsl:template match="/">
    ...
    <div id="mir-citation">
      ...
      <xsl:if test="$MIR.shariff = 'show'">
        <xsl:variable name="oid" select="mycoreobject/@ID" />
        <xsl:variable name="title" select="$mods/mods:titleInfo[not(@altFormat)][1]/mods:title" />
        
        <div class="shariff"  
          data-url="{$WebApplicationBaseURL}receive/{$oid}" 
          data-services="['mail', 'twitter', 'facebook', 'whatsapp', 'linkedin', 'xing', 'pinterest', 'info']" 
          data-mail-url="mailto:" 
          data-mail-subject="{i18n:translate('duepublico.shariff.subject')}: {$title}" 
          data-mail-body="{$WebApplicationBaseURL}receive/{$oid}" 
