...
<xsl:stylesheet version="3.0"
  ...
  <xsl:variable name="nameIdentifierTypes" select="document('classification:metadata:-1:children:nameIdentifier')/mycoreclass" />

  <xsl:template match="mods:nameIdentifier[(@type='gnd') or (@type='viaf') or (@type='orcid') or (@type='ror')]">
    <xsl:variable name="schemeURI" select="$nameIdentifierTypes//category[@ID=current()/@type]/label[@xml:lang='x-uri']/@text" />
    <xsl:variable name="schemeURIWithoutSlash" select="substring($schemeURI,1,string-length($schemeURI)-1)" />

    <nameIdentifier nameIdentifierScheme="{fn:upper-case(@type)}" schemeURI="{$schemeURIWithoutSlash}">
      <xsl:value-of select="concat($schemeURI,text())" />
    </nameIdentifier>
  </xsl:template>
