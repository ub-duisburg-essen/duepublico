...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink"
  ...
  <!-- List of elements to exclude from dynamic fields, comma separated -->
  <xsl:param name="MCR.Solr.DynamicFields.excludeElements" select="''" />
  ...
  <xsl:template match="mycoreobject">
    ...
    <xsl:if test="$MCR.Solr.DynamicFields='true' and $isExcluded = 'false'">
      ...
      <xsl:for-each select=".//*[not(*)]">
        ...
        <xsl:choose>
          <xsl:when test="contains(concat(',',$MCR.Solr.DynamicFields.excludeElements,','),concat(',',$element,','))">
            <!-- Element name is in list of elements to exclude from dynamic fields, so skip it -->
          </xsl:when>
